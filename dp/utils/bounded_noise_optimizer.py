...
def _retrieve_rdp_for_all(rdps, c_range, theta, orders):
    """
    Retrieve RDP budget for the summed gradient theta.
    Theta is rounded to one decimal place so that we can use precomputed rdp
    dictionary rdps for fast retrieval of RDP.

    Return: (2,\epsilon)-RDP, RDP for all orders
    ...
    c_min = c_range[0]
    c_max = c_range[-1]
    step = c_range[1] - c_range[0]
    dither_theta = np.clip(np.around(theta, decimals=1), a_min=c_min, a_max=c_max)
    theta_dithered_pos = ((dither_theta + c_min) / step).astype(int)
    return (rdps[9])[theta_dithered_pos], np.array([rdp[theta_dithered_pos].sum() for rdp in rdps])
...
def _truncate(
    ...
    a = bound
    theta_np = reference.cpu().detach().numpy()
    noisy_theta = truncnorm.rvs((-a-theta_np)/std,(a-theta_np)/std,loc=theta_np,scale=std)
    # print(device, reference.device)
    return torch.from_numpy(noisy_theta).view_as(reference).to(device)
...
class BoundedNoiseDPOptimizer(Optimizer):
    """
    ...
    def __init__(
        ...
        *,
        ...
        if rectification and truncation:
            raise ValueError("You should not use truncation and rectificatio at the same time.")
    ...
    def attach_step_hook(self, fn: Callable[[BoundedNoiseDPOptimizer], None]):
        """
    ...
    def clip_and_accumulate(self,L2_clipping=False):
        """
        ...
        for p in self.params:
            ...
            # print("Grad sample max: ", grad_sample.max(), "         Grad sample min: ", grad_sample.min(), "         Grad sample 25/75 quantile: ", torch.quantile(grad_sample,0.25),torch.quantile(grad_sample,0.75))
            # grad = torch.clamp(grad_sample,min=-self.max_grad_norm,max=self.max_grad_norm).sum(0)
    ...
    def pre_step(
        self, closure: Optional[Callable[[], float]] = None, random_project = False,
    ) -> Optional[float]:
        """
        ...
        """
        L2_clipping = False
        self.clip_and_accumulate(L2_clipping, random_project)
...
class BoundedNoiseDistributedDPOptimizer(BoundedNoiseDPOptimizer):
    """
    ...
    def reduce_gradients(self, rdps, c_range, orders, random_project=False):
        all_rdp_layers = 0
        counter = 0
        for p in self.params:
            ...
            print("Noisy grad sample max: ", p.grad.max()* self.expected_batch_size, "         Noisy grad sample min: ", p.grad.min()* self.expected_batch_size)

            counter += 1

            # RDP for rectified Gaussian and truncated Gaussian needs to be calculated based on the gradient value.
            if (self.truncation or self.rectification):
                rdp_layer_per_dim, rdp_layer = _retrieve_rdp_for_all(rdps, c_range, p.grad.flatten() * self.expected_batch_size, orders)
                all_rdp_layers += rdp_layer
            ...
            # Peform rectification
            if self.rectification:
                p.grad = torch.clamp(p.grad * self.expected_batch_size, min = -self.bound, max = self.bound)
                p.grad /= self.expected_batch_size   
            # Perform truncation
            elif self.truncation:
                trunc_grad = _truncate(
                    std=self.noise_multiplier * self.max_grad_norm,
                    bound=self.bound,
                    reference=p.grad * self.expected_batch_size,
                    device=p.device,
                )
                p.grad.data = trunc_grad.data.to(torch.float32)
                p.grad.data /= self.expected_batch_size
            if self.loss_reduction == "mean":
                p.grad.data /= self.world_size

        return all_rdp_layers


    def step(
        self, rdps, c_range, orders, closure: Optional[Callable[[], float]] = None, random_project = False,
    ) -> Optional[torch.Tensor]:
        if self.pre_step(random_project=random_project):
            rdp = self.reduce_gradients(rdps, c_range, orders, random_project=random_project)
            return rdp, self.original_optimizer.step(closure)
        else:
            rdp = 0
            return rdp, None
