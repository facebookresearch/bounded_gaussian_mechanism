...
def get_ds(args):
    ...
    # print(dataset_path, extracted_path)
    ...
    if args.dataset in ["ImageNet"]:
        ...
        PET_MEAN = (0.485, 0.456, 0.406)
        PET_STD_DEV = (0.229, 0.224, 0.225)
        transform = transforms.Compose([transforms.Resize((32,32)),transforms.RandomHorizontalFlip(),transforms.ToTensor(),transforms.Normalize(PET_MEAN, PET_STD_DEV)])
        train_ds = getattr(datasets, "OxfordIIITPet")(
            dataset_path, transform=transform, download=True
        )
        images_train = []
        for i in range(len(train_ds._labels)):
            images_train.append(train_ds[i][0].unsqueeze(0))
        images_train = torch.cat(images_train)
        labels_train = torch.tensor(train_ds._labels)

        # images_train, labels_train = torch.tensor(
        #     train_ds.data.transpose(0, 3, 1, 2)
        # ) / 255.0, torch.tensor(train_ds.targets)
        test_ds = getattr(datasets, "OxfordIIITPet")(
            dataset_path, transform=transform, split="test", download=True
        )
        images_test = []
        for i in range(len(test_ds._labels)):
            images_test.append(test_ds[i][0].unsqueeze(0))
        images_test = torch.cat(images_test)
        labels_test = torch.tensor(test_ds._labels)
        # images_test, labels_test = orch.tensor(
        ...
        ### Uncomment these when data file is saved
        # extracted_train_path = extracted_path + "/_train.npy"
        # labels_train_path = extracted_path + "/_train_labels.npy"
        # extracted_test_path = extracted_path + "/_test.npy"
        # labels_test_path = extracted_path + "/_test_labels.npy"
        ...
        train_ds = getattr(datasets, "CIFAR100")(
            dataset_path, transform=transform, train=True, download=True
        )
        images_train, labels_train = torch.tensor(
            train_ds.data.transpose(0, 3, 1, 2)
        ) / 255.0, torch.tensor(train_ds.targets)
        test_ds = getattr(datasets, "CIFAR100")(
            dataset_path, transform=transform, train=False, download=True
        )
        images_test, labels_test = torch.tensor(
            test_ds.data.transpose(0, 3, 1, 2)
        ) / 255.0, torch.tensor(test_ds.targets)
        ...
        extracted_test_path = extracted_path + "/_test.npy"
        labels_test_path = extracted_path + "/_test_labels.npy"
        if not os.path.exists(extracted_train_path_part_1) or not os.path.exists(extracted_train_path):
            ...
            ds = getattr(datasets, "CIFAR10")(
                dataset_path, transform=train_tr, train=True, download=True,
            )
            images_train, labels_train = torch.tensor(
            ...
            ds = getattr(datasets, "CIFAR10")(
                dataset_path, transform=test_tr, train=False, download=True
            )
            images_test, labels_test = torch.tensor(
                ds.data.transpose(0, 3, 1, 2)
            ) / 255.0, torch.tensor(ds.targets)
    ...
    return ds_train, train_loader, test_loader